...
| `/pages/local-my-missions.html`         | The local `my-missions` build on a hostile host (table + ProseMirror `[contenteditable]` leaks); authenticated — pass `?token=…&apiUrl=…` to view it as a real person                            |
...
| `/pages/local-event-registration.html`  | The local `event-registration` build on a hostile host (hotpink inputs, uppercase labels); `?id=<Event_ID>`, paste an MP token for the household path; phone width shows the sticky bars + sheet |
