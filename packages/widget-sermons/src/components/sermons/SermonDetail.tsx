...
export function SermonDetail({ id, config, onBack }: SermonDetailProps) {
    ...
    return (
        <div>
            <button type="button" onClick={onBack} className="flex items-center gap-1 text-sm text-primary mb-4"><ArrowLeft className="h-4 w-4" /> Back to sermons</button>
            <SkeletonTransition isLoading={isLoading} skeleton={<div className="space-y-4"><Skeleton className="h-8 w-2/3" /><Skeleton className="h-5 w-1/2" /><Skeleton variant="card" className="h-64 w-full" /><Skeleton className="h-24 w-full" /></div>}>
                {sermon && (
                    <div className="space-y-4">
                        <div>
                            ...
                            {sermon.scriptureLinks && <p className="text-xs text-stone-400 mt-1">Scripture: {sermon.scriptureLinks}</p>}
                        </div>
                        ...
                        <div className="flex items-center gap-3 rounded-lg bg-stone-50 p-4 dark:bg-stone-900">
                            <div className="flex h-12 w-12 items-center justify-center rounded-full bg-stone-200 text-sm font-semibold text-stone-600 dark:bg-stone-700 dark:text-stone-300">{getInitials(sermon.speaker.name)}</div>
                            <div>
