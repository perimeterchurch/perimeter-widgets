...
describe('ReleasePanel', () => {
  it('badges the live build and promotes a newer one on click', async () => {
    ...
    render(<ReleasePanel name="sermons" builds={builds} latest="1.0.0" onPromote={onPromote} onRollback={onRollback} />);
  ...
  it('rolls back to an older version when latest is newer', async () => {
    const onPromote = vi.fn().mockResolvedValue(undefined);
    const onRollback = vi.fn().mockResolvedValue(undefined);
    render(<ReleasePanel name="sermons" builds={builds} latest="1.1.0" onPromote={onPromote} onRollback={onRollback} />);
  ...
  it('promotes when nothing is live yet', async () => {
    const onPromote = vi.fn().mockResolvedValue(undefined);
    const onRollback = vi.fn().mockResolvedValue(undefined);
    render(<ReleasePanel name="sermons" builds={builds} latest={null} onPromote={onPromote} onRollback={onRollback} />);
