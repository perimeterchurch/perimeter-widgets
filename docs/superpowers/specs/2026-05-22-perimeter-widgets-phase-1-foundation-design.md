...
2. `pnpm build` produces `dist/example/index.js` (an IIFE) under the per-widget gzipped size budget (umbrella: 850 KB; raised from 680 KB during Phase 3 follow-up #8 after inlining the pdf.js worker).
...
| Bundle size already over budget before any real widget ships | Phase 1 sets up a CI check that fails the build if the example widget exceeds **850 KB gzipped** (matches the umbrella; raised twice since — Phase 2 lifted 500→680 KB after measuring the real sermons port at 629 KB gz, Phase 3 follow-up #8 lifted 680→850 KB after inlining the pdf.js worker). React 19 + ReactDOM alone are ~125 KB gz; with TanStack Query + zod + the runtime, a fresh example widget lands ~207 KB — well within budget. |
