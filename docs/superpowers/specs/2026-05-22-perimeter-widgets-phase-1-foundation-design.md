...
2. `pnpm build` produces `dist/example/index.js` (an IIFE) under the per-widget gzipped size budget (umbrella: 680 KB).
...
| Bundle size already over budget before any real widget ships | Phase 1 sets up a CI check that fails the build if the example widget exceeds **680 KB gzipped** (matches the umbrella; raised in Phase 2 after measuring the real sermons port at 629 KB gz). React 19 + ReactDOM alone are ~125 KB gz; with TanStack Query + zod + the runtime, a fresh example widget lands ~207 KB — well within budget. |
