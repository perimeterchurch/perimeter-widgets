...
| Bundle size growth with all-React-bundled-in | Per-widget budget: **850 KB gzipped** (raised from 680 KB during the Phase 3 follow-up that inlined the pdf.js worker into the sermons bundle — previously the worker was runtime-fetched from unpkg, so its ~280 KB-gz cost was off the books). Sermons now lands at ~801 KB gz. The Phase 1 platform stack alone is ~200 KB gz; sermons additionally bundles react-pdf (pdf.js worker + viewer, dominant cost), HLS.js, framer-motion, luxon, nuqs, headlessui, lucide-react. The single-IIFE delivery contract inlines dynamic imports, so lazy-loading defers init but not download. A follow-up optimization could move the pdf.js worker to a self-hosted same-origin URL on `widgets.perimeter.org` (with CORS headers) and drop the inlined copy, returning the bundle to the 500-600 KB range. If multiple widgets push past the budget, the loader-script path can be extended to share React across bundles — out of scope for this rebuild |
