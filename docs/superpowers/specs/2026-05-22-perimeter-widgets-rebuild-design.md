...
| Bundle size growth with all-React-bundled-in | Per-widget budget: **680 KB gzipped** (revised in Phase 2 after measuring the real sermons port at 629 KB gz — the legacy production widget was already 554 KB gz, so sermons has always been a ~550 KB+ widget). The Phase 1 platform stack alone is ~200 KB gz; sermons additionally bundles react-pdf (pdf.js, the dominant cost), HLS.js, framer-motion, luxon, nuqs, headlessui, lucide-react. The single-IIFE delivery contract inlines dynamic imports, so lazy-loading defers init but not download. A follow-up optimization task tracks the ~75 KB-gz growth over legacy. If multiple widgets push past the budget, the loader-script path can be extended to share React across bundles — out of scope for this rebuild |
