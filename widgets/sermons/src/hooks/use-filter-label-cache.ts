...
export type Dimension = 'speaker' | 'book' | 'series' | 'serviceType' | 'seriesType';
...
export interface FilterLabelCache {
  ...
  absorb(dimension: Dimension, options: { id: number; label: string }[]): void;
  /**
...
export function useFilterLabelCache(): FilterLabelCache {
  ...
  const absorb = useCallback((dimension: Dimension, options: { id: number; label: string }[]) => {
    ...
    (
      ...
      const missing = selectedIds
        ...
        .map((id) => ({
          value: String(id),
          label:
            cacheRef.current.get(dimension)?.get(id) ?? `${DIMENSION_SINGULAR[dimension]} ${id}`,
        }));
