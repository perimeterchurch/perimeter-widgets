import { describe, it, expect, beforeAll } from 'vitest';
import { execSync } from 'node:child_process';
import { gzipSync } from 'node:zlib';
import { readFileSync, existsSync, readdirSync } from 'node:fs';
import path from 'node:path';

const root = path.resolve(__dirname, '..');
const distDir = path.join(root, 'dist');
const bundle = path.join(distDir, 'index.js');

beforeAll(() => {
  // Vitest exports NODE_ENV=test, and Vite respects a pre-set NODE_ENV — the
  // child build would silently use the DEV jsx transform and ship a bundle
  // that crashes on real pages (_.jsxDEV is not a function). Pin production.
  execSync('pnpm exec vite build', {
    cwd: root,
    stdio: 'inherit',
    env: { ...process.env, NODE_ENV: 'production' },
  });
}, 120_000);

describe('built my-missions bundle', () => {
  it('emits a single IIFE at dist/index.js', () => {
    expect(existsSync(bundle)).toBe(true);
  });
  it('inlines CSS — no separate .css asset is emitted', () => {
    expect(readdirSync(distDir).some((f) => f.endsWith('.css'))).toBe(false);
  });
  it('self-mounts: bundle references the widget name and PerimeterWidgets global', () => {
    const code = readFileSync(bundle, 'utf8');
    expect(code).toContain('my-missions');
    expect(code).toContain('PerimeterWidgets');
  });
  it('is a production-transform build (no dev JSX runtime)', () => {
    // jsxDEV in the bundle means a non-production NODE_ENV leaked into the
    // build — the artifact crashes at runtime under the production define.
    expect(readFileSync(bundle, 'utf8')).not.toContain('jsxDEV');
  });

  it('stays inside its transfer budget', () => {
    // ~248 KB gz at the port, of which Tiptap/ProseMirror is a little under
    // half. `widgetConfig` emits ONE IIFE with no code splitting, so every
    // visitor downloads the letter editor whether or not they open My Letter —
    // which is why this is worth a guard rather than a note. The ceiling has
    // room for dependency drift; a jump past it means something new landed in
    // the bundle, so check what before raising it.
    const gzipped = gzipSync(readFileSync(bundle)).length;
    expect(gzipped).toBeLessThan(290 * 1024);
  });

  it('bundles the rich-text editor rather than fetching it at runtime', () => {
    // A CDN-hosted widget on an arbitrary host page can't rely on a second
    // network request resolving; ProseMirror has to be in the artifact.
    expect(readFileSync(bundle, 'utf8')).toContain('ProseMirror');
  });
});
